...
def require_access(level, message):
  ...
  if level_ == None or level_ < level:
    if message != '':
...
class Report(Frame):
  ...
  def refresh(self):
    ...
    treeScroll.place(x = 952, y = 101, height = 324)
    ...
    for col in self.columns:
      ...
      self.tree.heading(col, text = col, anchor = 'w', command = lambda _col=col: treeview_sort_column(self.tree, _col, False))
